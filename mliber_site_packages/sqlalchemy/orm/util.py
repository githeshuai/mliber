...
def polymorphic_union(
    ...
    :param aliasname: name of the :func:`~sqlalchemy.database_api.expression.alias()`
     construct generated.
...
def identity_key(*args, **kwargs):
    ...
    * ``identity_key(instance=instance)``
      ...
      effect of performing a database_api check for the corresponding row
    ...
    * ``identity_key(class, row=row, identity_token=token)``
      ...
      database_api result row as a :class:`.RowProxy` object.
...
class AliasedClass(object):
    ...
    The ORM equivalent of a :func:`~sqlalchemy.database_api.expression.alias`
    construct, this object mimics the mapped class using a
    ``__getattr__`` scheme and maintains a reference to a
    real :class:`~sqlalchemy.database_api.expression.Alias` object.
...
def join(
    ...
    provided by :func:`.database_api.expression.join()`, where the
...
def has_identity(object_):
    """Return True if the given object has a database_api
    identity.
